...
function Protected({ children }) {
  ...
  // While reading localStorage — show spinner, do NOT redirect yet
  if (loading) {
    return (
  ...
  // Loading done — if no user, go to login
  if (!user) {
      ...
      <div className="min-h-screen bg-slate-950 flex">
        ...
        <aside className="w-56 shrink-0 border-r border-slate-800/60 flex flex-col">
          ...
          <nav className="flex-1 px-3 py-4 space-y-1">
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-600 px-3 mb-2">
              Storage
            </p>
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          <header className="h-16 border-b border-slate-800/60 flex items-center px-6 gap-3">
            ...
            Replication Factor: 2 · Chunk Size: 1 MB
          </span>
          </header>
